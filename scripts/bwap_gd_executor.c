...
double run_app(float weights[]) {
  ...
  time(&start);
  system(
      "UNSTICKYMEM_MODE=scan BWAP_WEIGHTS=/home/dgureya/devs/unstickymem/config/weights_1w.txt numactl --physcpubind=0-15 /home/dgureya/parsec-3.0/ext/splash2x/apps/ocean_cp/inst/amd64-linux.gcc/bin/ocean_cp -n4098 -p16 -e1e-14 -r10000 -t14400");
...
int main(void) {
  ...
  for (n = 0; n < max_iter; n++) {
    ...
    for (j = 0; j < MAX_NODES; j++) {
      initial_weights[j] = (initial_weights[j] - (w_step * dim_deravative[j]));
