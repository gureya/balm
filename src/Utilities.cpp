...
void periodic_monitor() {
  ...
  while (run) {
    ...
    if (stall_rate.at(1) > target_stall_rate * 1.001) {
      ...
      if (current_remote_ratio != 0) {
        //Enforce MBA
        optimal_mba = apply_mba(target_stall_rate);
      ...
      //Optimize page migration either way!
      //
...
double get_target_stall_rate() {
  ...
  char min_buf[32];
  sprintf(min_buf, "sudo pqos -e 'mba@0:0=%d'", min_mba);
  system(min_buf);
  ...
  char max_buf[32];
  sprintf(max_buf, "sudo pqos -e 'mba@0:0=%d'", max_mba);
  system(max_buf);
...
/*
 * Apply the highest MBA that still meets the target SLO
...
int apply_mba(double target_stall_rate) {
  ...
  do {
    ...
    if (stall_rate.at(1) <= target_stall_rate) {
    ...
    progress = stall_rate.at(1) - target_stall_rate;
 ...
 * TODO: How about releasing the MBA
 *
 */
...
int apply_pagemigration_lr(double target_stall_rate, int current_remote_ratio) {
  ...
  for (i = current_remote_ratio; i <= 100; i += ADAPTATION_STEP) {
    ...
    best_stall_rate.at(0) = std::min(best_stall_rate.at(0), stall_rate.at(0));
    ...
    if (interval_diff.at(1) > minimum_interference.at(1)) {
      ...
      if (interval_diff_0 > minimum_interference_0) {
        ...
        if (i != 0) {
          ...
          //place_all_pages(mem_segments, (i - ADAPTATION_STEP));
          LINFOF("Final Ratio: %.2f", (i - ADAPTATION_STEP));
          remote_final_ratio = i - ADAPTATION_STEP;
        } else {
          LINFOF("Final Ratio: %.2f", i);
          remote_final_ratio = i;
        }
        LINFO(
